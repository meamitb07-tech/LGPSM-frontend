...
const DEFAULT_SYSTEM_USERS: SystemUserRow[] = [
  {
    id: "1",
    name: "Moloy Roy",
    ...
    assignedEvents: [
      ...
      },
      {
        ...
        sessions: [
          { id: "s3", name: "DINNER SESSION", time: "08:30 PM TO 11:30 PM" },
        ],
      },
    ],
  },
...
export default function AssignSystemUsersPage() {
  const { user } = useAuth();
  const [users, setUsers] = useState<SystemUserRow[]>(DEFAULT_SYSTEM_USERS);
  const [expandedUserIds, setExpandedUserIds] = useState<string[]>(["1"]); // Default expand Moloy Roy matching Screenshot 1
  const [searchQuery, setSearchQuery] = useState("");
  // Pre-selected user checkboxes matching reference screenshots
  ...
  const fetchUsers = async () => {
    ...
    DEFAULT_SYSTEM_USERS.forEach((def) => {
        ...
        <header className="h-20 bg-white border-b border-gray-200 px-6 sm:px-8 flex items-center justify-between sticky top-0 z-20 shrink-0">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-gray-100 hover:bg-gray-200/80 px-3 py-1.5 rounded-full cursor-pointer transition-colors">
              ...
              <span className="text-xs font-semibold text-gray-800">{user?.fullName || user?.email || "Jane Doe"}</span>
